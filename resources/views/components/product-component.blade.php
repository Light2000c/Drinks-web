    ...
    <div class="single_product">
        ...
        <div class="product_content grid_content">
            ...
            <div class="action_links">
                <div
                    class="row g-0 display-flex justify-content-center removewrapper{{ $product->id }}">
                    <div class="col-2">
                        <button onclick="increment({{ $product->id }})" type="button"
                            class="btn btn-outline btn-dark" style="border-radius: 18px"
                            title="add to cart"><i class="fa fa-plus"></i> </button>
                    </div>
                    <div class="col-3">
                        <input onchange="updateQuantity({{ $product->id}}, {{ Auth::user()->id }} )" class="form-control"
                            name="input{{ $product->id }}"
                            value="{{ $product->cartQuantity(Auth::user()) }}"
                            type="number">
                    </div>
                    <div class="col-2">
                        <button onclick="decrement({{ $product->id }})" type="button" class="btn btn-outline btn-dark"
                            style="border-radius: 18px" title="add to cart"><i
                                class="fa fa-minus"></i></button>
                    </div>
                    <div class="col-2">
                        <button type="submit"
                            class="btn btn-outline btn-dark remove{{ $product->id }}"
                            style="border-radius: 18px" title="add to cart"><i
                ...
                <ul class="addwrapper{{ $product->id }}">
                    ...
                    <li class="add_to_cart">
                        <button type="button"
                            class="btn btn-outline btn-dark add{{ $product->id }}"
                            style="border-radius: 18px" title="add to cart"><i
